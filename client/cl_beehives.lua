        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["np-taskbar"]:taskBar(15000, "Installing beehive", false, true, false, false, nil, 5.0, PlayerPedId())
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    if zone == "np-beekeeping:bee_zone" then
        print("Entering bee zone")
        inZone = inZone + 1
        if inZone == 1 then
            print("Getting beehives")
            --setDeleteAll = false
